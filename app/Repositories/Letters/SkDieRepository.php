...
class SkDieRepository
{
  public function __construct(
    protected readonly Sk $sk,
    protected readonly SkDieLetter $letter,
  ...
  public function findById(SkDieLetter $letter): SkDieLetter
  {
    return $this->letter
      ->where('id', $letter->id)
      ->with(['sk.villageHead', 'sk.environmentalHead', 'sk.sectionHead', 'sk.citizent'])
  ...
  public function update($request, SkDieLetter $letter): bool|array|Exception
  {
    DB::beginTransaction();
  ...
  public function confirmationLetter(SkDieLetter $letter, $status, $reject_reason = null): bool|Exception
  {
    DB::beginTransaction();
    try {
  ...
  public function delete(SkDieLetter $letter): bool|array|Exception
  {
    DB::beginTransaction();
    try {
