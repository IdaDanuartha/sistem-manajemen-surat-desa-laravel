...
@section('main')
	...
	<div class="table-wrapper mt-[20px] input-teacher">
		...
		<form class="grid grid-cols-12 gap-4">						
			...
			@endif	
			@if (auth()->user()->isSectionHead() || auth()->user()->isVillageHead() || auth()->user()->isEnvironmentalHead())
			<div class="col-span-12 flex flex-col">
			...
			<div class="col-span-12  flex justify-between">
				<div class="flex items-center gap-3">				
					<a href="{{ route('letters.sk-domicile.index') }}" class="button btn-second text-white" type="reset">Kembali</a>
					<a href="{{ route('citizents.show', $get_letter->sk->citizent_id) }}" target="_blank" class="button bg-gray-500 text-white">Profile Warga</a>
					<a href="{{ route('letters.sk-domicile.preview', $get_letter->id) }}" target="_blank" class="button btn-main text-white">Preview Surat</a>
				</div>
				<div class="flex items-center">
					<a href="{{ route('letters.sk-domicile.download', $get_letter->id) }}" target="_blank" class="button btn-second mr-2 text-white">Download PDF</a>
				</div>
			</div>
		</form>
	</div>
...
<x-modal-approve-letter>
	<x-slot name="route">/letters/sk-domicile/{{ $get_letter->id }}/approve</x-slot>
</x-modal-approve-letter>
<x-modal-reject-letter>
	<x-slot name="route">/letters/sk-domicile/{{ $get_letter->id }}/reject</x-slot>
</x-modal-reject-letter>
@endsection
