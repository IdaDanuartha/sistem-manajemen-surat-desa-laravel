...
@section('main')
	...
	<div class="table-wrapper mt-[20px] input-teacher">
		...
		<form class="grid grid-cols-12 gap-4">						
			...
			<div class="col-span-12 md:col-span-6 flex flex-col">
                <label for="" class="text-second mb-1">Nama Warga yang Meninggal</label>
                <input type="text" class="input-crud" value="{{ $get_letter->citizent->name }}" disabled />
            </div>
			<div class="col-span-12 md:col-span-6 flex flex-col">
                <label for="" class="text-second mb-1">Tahun Meninggal</label>
                <input type="text" class="input-crud" value="{{ $get_letter->year }}" disabled />
            </div>
			...
			<div class="col-span-12  flex justify-between">
				<div class="flex items-center gap-3">				
					<a href="{{ route('letters.sk-die.index') }}" class="button btn-second text-white" type="reset">Kembali</a>
					<a href="{{ route('citizents.show', $get_letter->sk->citizent_id) }}" target="_blank" class="button bg-gray-500 text-white">Profile Warga</a>
					<a href="{{ route('letters.sk-die.preview', $get_letter->id) }}" target="_blank" class="button btn-main text-white">Preview Surat</a>
				</div>
				<div class="flex items-center">
					<a href="{{ route('letters.sk-die.download', $get_letter->id) }}" target="_blank" class="button btn-second mr-2 text-white">Download PDF</a>
				</div>
			</div>
		</form>
	</div>
...
<x-modal-approve-letter>
	<x-slot name="route">/letters/sk-die/{{ $get_letter->id }}/approve</x-slot>
</x-modal-approve-letter>
<x-modal-reject-letter>
	<x-slot name="route">/letters/sk-die/{{ $get_letter->id }}/reject</x-slot>
</x-modal-reject-letter>
@endsection
