	...
	<div class="table-wrapper mt-[20px] input-teacher">
		...
		<form class="grid grid-cols-12 gap-4">						
			...
			<div class="col-span-12 md:col-span-4 flex flex-col">
                <label for="" class="text-second mb-1">Nama Pasangan</label>
                <input type="text" class="input-crud" value="{{ $get_letter->citizent->name }}" disabled />
            </div>
			<div class="col-span-12 md:col-span-4 flex flex-col">
                <label for="" class="text-second mb-1">Status</label>
                <input type="text" class="input-crud" value="@if($get_letter->status == 1) Duda @elseif($get_letter->status == 2) Janda @else Cerai @endif" disabled />
            </div>
			<div class="col-span-12 md:col-span-4 flex flex-col">
                <label for="" class="text-second mb-1">Tanggal</label>
                <input type="text" class="input-crud" value="{{ $get_letter->date->format("d M Y") }}" disabled />
            </div>
            @if (auth()->user()->isCitizent())
				<div class="col-span-12 md:col-span-4 flex flex-col">
			...
			<div class="col-span-12  flex justify-between">
				<div class="flex items-center gap-3">				
					<a href="{{ route('letters.sk-marital-status.preview', $get_letter->id) }}" target="_blank" class="button btn-main text-white">Preview Surat</a>
					<a href="{{ route('letters.sk-marital-status.index') }}" class="button btn-second text-white" type="reset">Kembali</a>
				</div>
				<div class="flex items-center">
					<a href="{{ route('letters.sk-marital-status.download', $get_letter->id) }}" target="_blank" class="button btn-second mr-2 text-white">Download PDF</a>
				</div>
			</div>
		</form>
	</div>
...
<x-modal-approve-letter>
	<x-slot name="route">/letters/sk-marital-status/{{ $get_letter->id }}/approve</x-slot>
</x-modal-approve-letter>
<x-modal-reject-letter>
	<x-slot name="route">/letters/sk-marital-status/{{ $get_letter->id }}/reject</x-slot>
</x-modal-reject-letter>
@endsection
