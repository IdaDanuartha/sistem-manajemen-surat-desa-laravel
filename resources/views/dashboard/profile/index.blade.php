...
<div class="mt-[20px] p-0 flex gap-5 lg:flex-row flex-col">
    <div class="table-wrapper p-[18px] w-full h-fit md:max-w-[300px]">
        @if (auth()->user()->authenticatable->profile_image ?? auth()->user()->authenticatable->citizent->profile_image)
            <img src="{{ asset('uploads/users/' . auth()->user()->authenticatable->profile_image ?? auth()->user()->authenticatable->citizent->profile_image) }}" alt="Profile Image" class="rounded w-full aspect-square object-cover object-center h-auto"/>
        @else
        ...
        <div class="row">
            ...
            @if(!auth()->user()->isAdmin())
                ...
                <div class="col-md-6 col-12 mb-4 flex flex-col">
                    ...
                    <input type="text" class="input-crud" value="{{ auth()->user()->authenticatable->gender ?? auth()->user()->authenticatable->citizent->gender->label() }}" readonly />
                </div>
                <div class="col-md-6 col-12 mb-4 flex flex-col">
                    <label class="text-second">Golongan Darah</label>
                    <input type="text" class="input-crud" value="{{ auth()->user()->authenticatable->blood_group ?? auth()->user()->authenticatable->citizent->blood_group->label() }}" readonly />
                </div>
                <div class="col-md-6 col-12 mb-4 flex flex-col">
                    <label class="text-second">Agama</label>
                    <input type="text" class="input-crud" value="{{ auth()->user()->authenticatable->religion ?? auth()->user()->authenticatable->citizent->religion->label() }}" readonly />
                </div>
                <div class="col-md-6 col-12 mb-4 flex flex-col">
                    <label class="text-second">Status Pernikahan</label>
                    <input type="text" class="input-crud" value="{{ auth()->user()->authenticatable->marital_status ?? auth()->user()->authenticatable->citizent->marital_status->label() }}" readonly />
                </div>
            @endif
            <div class="col-12">
