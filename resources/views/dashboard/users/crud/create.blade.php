        ...
        @enderror
			...
			<div class="col-span-12 md:col-span-6 flex flex-col">
				...
				<input
					type="number"
					class="input-crud"
					placeholder="Masukkan Nomor Induk Kependudukan Warga..."
					required
        ...
        @enderror
			...
			<div class="col-span-12 md:col-span-6 flex flex-col">
				...
				<input
					type="number"
					class="input-crud"
					placeholder="Masukkan Nomor Kartu Keluarga Warga..."
					required
        ...
        @enderror
			...
			<div class="col-span-12 md:col-span-6 flex flex-col">
				...
				<input
					type="number"
					class="input-crud"
					placeholder="Masukkan Nomor Telepon Warga..."
					required
					id="phone_number"
          name="phone_number"
        ...
        @enderror
			...
			<div class="col-span-12 md:col-span-6 flex flex-col">
				...
				@foreach (App\Enums\Gender::labels() as $key => $value)
					<option value="{{ $key+1 }}" class="capitalize" @selected(old('gender') == $key+1)>{{ $value }}</option>
				@endforeach
				</select>
				@error('gender')
					<div class="text-danger mt-1">{{ $message }}</div>
			...
			<div class="col-span-12 md:col-span-6 flex flex-col">
				...
				@foreach (App\Enums\BloodGroup::labels() as $key => $value)
					<option value="{{ $key+1 }}" class="capitalize" @selected(old('blood_group') == $key+1)>{{ $value }}</option>
				@endforeach
				</select>
				@error('blood_group')
					<div class="text-danger mt-1">{{ $message }}</div>
			...
			<div class="col-span-12 md:col-span-6 flex flex-col">
				...
				@foreach (App\Enums\Religion::labels() as $key => $value)
					<option value="{{ $key+1 }}" class="capitalize" @selected(old('religion') == $key+1)>{{ $value }}</option>
				@endforeach
				</select>
				@error('religion')
					<div class="text-danger mt-1">{{ $message }}</div>
			...
			<div class="col-span-12 md:col-span-6 flex flex-col">
				...
				@foreach (App\Enums\MaritalStatus::labels() as $key => $value)
					<option value="{{ $key+1 }}" class="capitalize" @selected(old('marital_status') == $key+1)>{{ $value }}</option>
				@endforeach
				</select>
			...
			<div class="col-span-12 flex flex-col">
				...
				<label for="profile_image" class="d-block mb-3">
					<img src="{{ asset('assets/img/upload-image.jpg') }}" class="create-citizent-preview-img border" width="300" alt="">
				</label>
				<input
					type="file"
					id="profile_image"
			...
			<div class="col-span-12 flex items-center gap-3 mt-2">
				...
				<a href="{{ route('users.index') }}" class="button btn-second text-white" type="reset">Batal Tambah</a>
			</div>
		</form>
	</div>
@endsection
