    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-head" id="delete{{ $modalId }}ModalTitle">Hapus <span class="capitalize">{{ $name }}</span></h5>
          <button
            type="button"
        ...
        <div class="modal-body">
          <p>
                      Konfirmasi Penghapusan <span class="capitalize">{{ $name }}</span>: Apakah Anda yakin ingin menghapus
                      {{ $name }}? Tindakan ini tidak dapat dibatalkan dan <span class="lowercase">{{ $name }}</span> akan
                      dihapus secara permanen dari sistem.
                  </p>
        </div>
        <form action="" method="POST" id="delete_environmental_form" class="d-flex justify-content-center">
          @csrf
          @method("DELETE")
            <div class="flex justify-content-center gap-3 mb-4 w-100 px-4">
              <button type="submit" class="button btn-main w-100 btn-delete-modal">Hapus <span class="capitalize">{{ $name }}</span></button>
